...
3. **SQL and catalogs.** Close remaining language, type, privilege, collation, and introspection differences through strict PostgreSQL differential tests. Full-table index keys retain bounded value caches; `INCLUDE` columns are typed covering metadata, never key columns, and partial-index membership is authoritative through transactions, WAL, checkpoints, catalogs, and uniqueness checks. Index renames are transaction-visible identities with WAL recovery and checkpoint durability. Global, tablespace, and concurrent variants remain explicit gaps. Aggregate window `FILTER` is supported; PostgreSQL rejects `DISTINCT` and argument ordering in a window call.
...
3. **DDL and catalogs.** Typed column, domain, enum, sequence, schema, view, routine, and index definitions are transaction-owned state across DDL, WAL, recovery, catalogs, and query description. Index renames and partial predicates follow the same path through rollback, checkpoints, and `pg_indexes`; remaining catalog work is broader PostgreSQL coverage. Functions and procedures retain overload identity, owner, and ACLs across replacement, identity changes, recovery, and introspection. Synthesized results are statement-arena bounded and fail on exhaustion.
