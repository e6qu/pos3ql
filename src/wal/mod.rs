...
fn encoded_payload_len(op: &WalOp) -> usize {
    match op {
        WalOp::CreateTable(def) => {
            ...
            // checks
            n += 1;
            for ck in def.checks() {
                n += 1 + ck.name.as_str().len() + 2 + ck.expr.as_str().len();
            }
            // foreign keys
            n += 1;
            for fk in def.fkeys() {
...
fn append_payload(buf: &mut FixedBuf, op: &WalOp) -> bool {
    ...
    match op {
        WalOp::CreateTable(def) => {
            ...
            for ck in def.checks() {
                ok &= name_bytes(buf, ck.name.as_str());
                let e = ck.expr.as_str();
    ...
    match kind {
        KIND_CREATE => {
            ...
            for k in 0..n_checks {
                ...
                let mut ck = CheckConstraint::EMPTY;
                ck.name = SqlName::parse(cname).ok()?;
                core::fmt::Write::write_str(&mut ck.expr, text).ok()?;
...
fn code_type(code: u8) -> Option<ColType> {
    Some(match code {
        ...
        18 => ColType::Json,
        19 => ColType::Jsonb,
        c if c >= 32 => match crate::sql::types::ArrElem::from_code(c - 32) { Some(e) => ColType::Array(e), None => return None },
        9 => ColType::Uuid,
        10 => ColType::Bytea,
        11 => ColType::Numeric,
        _ => return None,
...
mod tests {
    ...
    fn sample_def() -> TableDef {
        ...
        def.n_uniques = 1;
        let mut ck = CheckConstraint::EMPTY;
        ck.name = SqlName::parse("t_check").unwrap();
        core::fmt::Write::write_str(&mut ck.expr, "id > 0").unwrap();
        def.checks[0] = ck;
        def.n_checks = 1;
        let mut fk = ForeignKey::EMPTY;
