...
| WAL compatibility | Pos3ql WAL is durable, checksummed, LSN-ordered, and cold-recoverable from object storage; an object-store upload failure fences subsequent statements and startup reconciliation must complete before admission. Each committed nonempty transaction now ends in an explicit durable boundary, so recovery and replication consumers cannot mistake a torn operation tail for a committed transaction. Replication startup is explicitly authorized; logical sessions retain PostgreSQL's simple-query SQL capability while both replication modes expose `IDENTIFY_SYSTEM` with the protocol's exact types; and the wire has a distinct CopyBoth response. Publications have transactional DDL, catalog rows, journal replay, and checkpoint/cold-start recovery. | Implement the logical publisher stream (`START_REPLICATION`, `pgoutput`) over those durable publications and transaction boundaries, then subscriber migration. PostgreSQL physical XLOG remains outside this design. |
