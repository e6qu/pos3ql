...
pub fn cast_to<'a>(v: Datum<'a>, target: ColType, arena: &'a Arena) -> Result<Datum<'a>, SqlError> {
    ...
    let out = match target {
        ...
        ColType::AnyArray => return Err(cast_unsupported(&v, "anyarray")),
        ...
        ColType::PgLsn => match v {
            Datum::PgLsn(_) => v,
            Datum::Text(s) => Datum::PgLsn(crate::sql::lsn::parse(s)?),
            _ => return Err(cast_unsupported(&v, "pg_lsn")),
        },
        ColType::AclItem => match v {
            Datum::AclItem(_) => v,
            Datum::Text(value) => Datum::Text(crate::sql::acl::canonicalize(value, arena)?),
            _ => return Err(cast_unsupported(&v, "aclitem")),
        },
