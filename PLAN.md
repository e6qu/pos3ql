...
3. **DDL and catalogs.** In progress: bytewise collations; typed constraint metadata; ACL-derived relation and usage grants; and sequence metadata match PostgreSQL. Column, domain, enum, and sequence definitions are transaction-owned typed state across DDL, WAL, checkpoints, and catalog rendering; staged sequence values follow the same ownership through savepoints. Login verification has explicit bootstrap, stored-role, and rejection states. Finish remaining client-tool introspection.
