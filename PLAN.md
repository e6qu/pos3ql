...
3. **SQL and catalogs.** Close remaining language, type, privilege, collation, and introspection differences through strict PostgreSQL differential tests. Index rebuilds reconstruct bounded value caches from authoritative rows for index, table, and schema scopes; global, tablespace, and concurrent variants remain explicit gaps. Aggregate window `FILTER` is supported; PostgreSQL rejects `DISTINCT` and argument ordering in a window call.
...
3. **DDL and catalogs.** Typed column, domain, enum, sequence, schema, view, and SQL-routine definitions are transaction-owned state across DDL, WAL, recovery, catalogs, and query description. Domain rename/schema moves retain dependent columns and child domains through rollback, WAL recovery, and checkpoints. Functions and procedures are distinct typed invocation contracts; `CALL` executes one durable SQL procedure statement in the caller transaction. Routine replacement, ownership changes, renames, schema moves, and generic drops preserve overload identity, owner, and ACLs across rollback, WAL recovery, checkpoints, `pg_proc`, and information-schema views. Overload-safe `GRANT`/`REVOKE`, default function privileges, and execution checks use that same identity. Synthesized results are statement-arena bounded and fail on exhaustion. Remaining catalog work is broader PostgreSQL coverage.
