...
3. **DDL and catalogs.** In progress: bytewise collations; typed constraint metadata; ACL-derived relation and usage grants; sequence metadata; and intrinsic routine identity match PostgreSQL. Column, domain, enum, sequence, schema, and view definitions are transaction-owned typed state across DDL, WAL, checkpoints, catalog rendering, and view description; staged sequence values follow the same ownership through savepoints. `information_schema` columns, domains, constraints, type usage, collations, roles, column privileges, views, and view table/column usage render from that state. View `_RETURN` identities and column bindings survive WAL and checkpoints and are catalog-visible. Synthesized results are statement-arena bounded rather than silently capped. `pg_proc` and visibility predicates derive from one typed intrinsic registry, and catalog OIDs retain OID wire identity. Finish stored routine DDL, routine/parameter views, expanded view sources, and canonical rule trees.
