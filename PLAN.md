...
3. **SQL and catalogs.** Close remaining language, type, privilege, collation, and introspection differences through strict PostgreSQL differential tests. Index rebuilds reconstruct bounded value caches from authoritative rows for index, table, and schema scopes; index renames are transaction-visible identities with WAL recovery and checkpoint durability. Global, tablespace, and concurrent variants remain explicit gaps. Aggregate window `FILTER` is supported; PostgreSQL rejects `DISTINCT` and argument ordering in a window call.
...
3. **DDL and catalogs.** Typed column, domain, enum, sequence, schema, view, SQL-routine, and index identities are transaction-owned state across DDL, WAL, recovery, catalogs, and query description. Index `RENAME TO` follows the same path through rollback, checkpoint, and `pg_indexes`; remaining catalog work is broader PostgreSQL coverage. Functions and procedures retain overload identity, owner, and ACLs across replacement, identity changes, recovery, and introspection. Synthesized results are statement-arena bounded and fail on exhaustion.
