...
| PostgreSQL binary client format | COPY binary is byte-exact for implemented stored types, including composites; binary Bind and Result formats now carry byte-exact built-in ranges, multiranges, and anonymous records as well as the scalar/array surface. | Expand binary type coverage with the SQL surface, including named composite types when they become storable catalog objects. |
