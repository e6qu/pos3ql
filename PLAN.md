...
| WAL compatibility | Pos3ql WAL is durable, checksummed, LSN-ordered, and cold-recoverable from object storage; an object-store upload failure fences subsequent statements and startup reconciliation must complete before admission. Replication startup is explicitly authorized, `IDENTIFY_SYSTEM` exposes the stable durable-namespace identity, and the wire has a distinct CopyBoth response. | Implement logical replication publisher (`START_REPLICATION`, `pgoutput`, publications), then subscriber migration. PostgreSQL physical XLOG remains outside this design. |
