...
body wins and releases its sibling. The remaining Pillars 2–4 work is physical
PAX-column late materialization and the vector operators that consume it; the
immutable packed range container is described below.
...
**PAX row-group slice (2026-08-03).** Checkpoint and compaction writers now
emit a self-describing column-group block for every table SST. Each group keeps
the ordered `(rowid, commit_lsn, tombstone)` stream beside per-column null maps
and the canonical physical value bytes; readers validate the complete group and
...
**Packed PAX range-read slice (2026-08-04).** PAX data groups now enter an
immutable `SstPackedContainerV1`: a bounded container holds several complete,
independently framed logical groups. The version-three SST index names each
logical group's container, byte extent, and content identity. A cold object
reader therefore requests exactly the named HTTP byte range, verifies the
returned logical frame before use, and caches that logical block rather than
the unrelated container bytes. The same operation passes through RAM and disk
...
fixed per-base-table proof for every bounded non-derived range-table shape:
join ON, WHERE, projection, grouping, ordering, and aggregate consumers
contribute the columns each source may expose. The bounded hash join carries
...
| PostgreSQL text SQL and catalogs | Broad SQL, DDL/DML, transactions, catalogs, dump/restore, and a zero-unsupported measured replay are implemented. Catalog-facing strict set-returning functions preserve PostgreSQL NULL semantics, so optional catalog values compose with real `pg_dump` queries instead of being mistaken for wrong argument types. | Continue strict differential expansion until the intended PostgreSQL language, catalog, locking, and tooling surface is covered; retain loud errors for every unsupported feature. |
| PostgreSQL binary client format | COPY binary is byte-exact for implemented stored types, including composites; binary Bind and Result formats now carry byte-exact built-in ranges, multiranges, and anonymous records as well as the scalar/array surface. Materialized query paths reconstruct structural records before response encoding, preserving that guarantee through sorting, grouping, and FROM-less execution. | Expand binary type coverage with the SQL surface, including named composite types when they become storable catalog objects. |
| WAL compatibility | Pos3ql WAL is durable, checksummed, LSN-ordered, and cold-recoverable from object storage; an object-store upload failure fences subsequent statements and startup reconciliation must complete before admission. Each committed nonempty transaction carries an explicit durable boundary and transaction identity; bounded cursors yield only whole verified transactions. Replication startup is explicitly authorized; logical sessions retain PostgreSQL's simple-query SQL capability while both replication modes expose `IDENTIFY_SYSTEM` with the protocol's exact types. Publications and bounded logical replication slots have journal replay, checkpoint/cold-start recovery, strict create/drop lifecycle, and catalog surfaces. `START_REPLICATION` enters CopyBoth at the greater of the requested LSN and slot confirmation, streams the retained object-store WAL before the local tail through one pgoutput encoder, and applies the fixed-capacity union of every named publication's table membership and operation flags. Its parser accepts standard unquoted and double-quoted publication identifiers, including commas and doubled quotes inside a quoted name, folds unquoted ASCII names, and retains the decoded selection in startup-sized connection storage bounded by `max_tables`. Only a transaction containing a selected Insert/Update/Delete/TRUNCATE reaches pgoutput; catalog, slot, and excluded-table WAL remain durable cursor progress without synthetic plugin framing. It emits selected tuples with full replica identity, validates complete ordered standby status frontiers before advancing a slot, and journals slot advancement before accepting it. The stream separates its connection-local send cursor from durable confirmation, so an unacknowledged transaction is never re-emitted and slot-advance bookkeeping never crosses pgoutput. An idle publisher has a reactor deadline and emits PostgreSQL `k` keepalives; an `r` reply request receives an immediate requested-reply frame. The object-store cursor reconstructs the same CRC-checked complete WAL frames as the local cursor before decoding. Pgoutput negotiates protocol v1 or v2 and optional binary tuples; v2 carries statement-level TRUNCATE with CASCADE/RESTART IDENTITY bits from an explicit transactional WAL event, while v1 rejects that event instead of changing it into row deletes. Type frames for user-defined enum/domain OIDs precede each Relation frame, and Relations truthfully advertise both those OIDs and the full replica identity carried by update/delete messages. | Add later pgoutput protocol versions, relation/schema DDL messages, subscriber migration, and an external vanilla-PostgreSQL logical-replication corpus. PostgreSQL physical XLOG remains outside this design. |
...
| Availability | VSR state machine and deterministic fault simulation exist. The SQL server does not yet consume live VSR topology, so `cluster` and `replica_id` are rejected configuration keys rather than inert state. | Productionize live write routing, quorum ordering/failover, and group commit without changing bucket-durable acknowledgement; introduce a typed cluster configuration only with that consumer. |
