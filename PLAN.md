...
work is to make grouped aggregates (including DISTINCT/ordered collectors), set
operations, windows, and nested materialized row sources consume/produce the
same run abstraction. Pillar 2 must then make its cursor suspendable around a
fixed in-flight GET pool; pillars 3–4 batch expression evaluation and introduce
PAX column-range late materialization.
