...
- SQL function query programs cross the projected-row codec; every read-only query runs, the final query alone returns rows, scalar calls cannot resolve set-returning definitions, and table calls cannot resolve scalar definitions. `RETURNS TABLE` owns ordered result names and types through replacement, WAL, checkpoints, catalogs, and scans.
