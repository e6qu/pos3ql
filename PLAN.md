...
4. **The map spills (gap 2)** — block-resident row index; secondary indexes
   ...
   **Uniqueness enforcement is now indexed (B-169, fixed 2026-07-25).** The
   ...
   error past it — the honest price of an in-RAM index. **Remaining (the
   ...
   spill unboundedly. Its format must supply multiset merge semantics (a
   tombstone for one `(value, rowid)` pair must not shadow another row with the
   same value), snapshot visibility, and planner probes over the same
   suspendable row source. A `CREATE UNIQUE INDEX` keeps its
...
- **Snapshot checkpoints instead of a leveled LSM** — *superseded.* Stages
  ...
  what remains dataset-size-bound is the in-RAM value index for constrained
  tables — gap 2 of the maturity roadmap above.
