...
2. **Closed: the row map is a cache overlay.** Cold committed entries and row
   ...
   set, not the durable dataset. The in-RAM value index is likewise only a
   bounded acceleration cache: exhaustion falls back to the object-resident
   row image and never limits correctness or table size. The remaining
   performance gap is its persistent value-keyed SST forest.
...
2. **Durability to the bucket (gaps 1 + 3)** — group-commit WAL-segment PUT
   ...
   **Status (2026-07-24): landed as defaults + the sliced checkpoint.**
   Commit-durable-on-bucket is now the *default* whenever
   `object_store = on`
...
4. **The map spills (gap 2)** — block-resident row index; secondary indexes
   ...
   **LZ4-compressed** when it pays (a hand-rolled implementation of the
   ...
   thereby closed.)
   **Status (2026-07-29): value indexes became caches, not authorities.**
   ...
   coercions. The SQLLogicTest unsupported count is a CI ratchet (183 per
   current shard), so an unsupported regression no longer hides behind the
   zero-divergence gate.

   **Remaining (the full persistent secondary-index LSM forest, "Design B"):**
   cache incompleteness still makes a value probe scan the object-resident row
   image. A value-keyed index persisted through the provider-neutral block
   stack must remove that object-scale scan. Its format must supply multiset
   merge semantics (a tombstone for one `(value, rowid)` pair must not shadow
   another row with the same value), commit-LSN snapshot visibility, bounded
   compaction, atomic manifest publication, cold-cache recovery, and planner
   probes over the same suspendable row source. RAM and local disk may cache
   those index blocks; neither may become authoritative.
...
- **Snapshot checkpoints instead of a leveled LSM** — *superseded.* Stages
  ...
  and the in-RAM value index is a bounded cache that falls through when
  incomplete. What remains dataset-size-sensitive is probe latency until the
  persistent value-keyed SST forest closes gap 2 of the maturity roadmap.
- **Checkpoint object-store calls are synchronous** — *superseded by the sliced
  ...
  design. WAL-segment upload is synchronous-by-default with object storage on
