...
-- MERGE uses the same DEFAULT and identity rules as INSERT and UPDATE. A
-- DEFAULT marker is an assignment state, never an expression that happens to
-- spell "default".
CREATE TABLE mg_defaults (
  id integer GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  value integer DEFAULT 41,
  generated integer GENERATED ALWAYS AS (value + 1) STORED
);
CREATE TABLE mg_defaults_source (id integer, value integer);
INSERT INTO mg_defaults_source VALUES (100, 7), (101, 8);
MERGE INTO mg_defaults AS target USING mg_defaults_source AS source ON target.id = source.id
  WHEN NOT MATCHED THEN INSERT (id, value) OVERRIDING SYSTEM VALUE VALUES (source.id, DEFAULT);
MERGE INTO mg_defaults AS target USING (VALUES (100)) AS source(id) ON target.id = source.id
  WHEN MATCHED THEN UPDATE SET value = DEFAULT;
SELECT id, value, generated FROM mg_defaults ORDER BY id;
DROP TABLE mg_defaults_source;
DROP TABLE mg_defaults;

-- `DEFAULT` on a serial or identity assignment consumes the sequence in both
-- ordinary UPDATE and MERGE UPDATE.
CREATE TABLE mg_default_identity (id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY);
INSERT INTO mg_default_identity DEFAULT VALUES;
UPDATE mg_default_identity SET id = DEFAULT WHERE id = 1;
MERGE INTO mg_default_identity AS target USING (VALUES (2)) AS source(id) ON target.id = source.id
  WHEN MATCHED THEN UPDATE SET id = DEFAULT;
SELECT id FROM mg_default_identity;
DROP TABLE mg_default_identity;

-- The target and source preserve their independent inheritance selections.
-- Default target/source references include descendants; ONLY changes one side
-- without changing the logical target of a later insert action.
CREATE TABLE mg_inherit_source (id integer PRIMARY KEY, value integer);
CREATE TABLE mg_inherit_source_child (child_marker integer) INHERITS (mg_inherit_source);
CREATE TABLE mg_inherit_target (id integer PRIMARY KEY, value integer);
CREATE TABLE mg_inherit_target_child (child_marker integer) INHERITS (mg_inherit_target);
INSERT INTO mg_inherit_source VALUES (1, 10);
INSERT INTO mg_inherit_source_child VALUES (2, 20);
INSERT INTO mg_inherit_target_child VALUES (1, 0), (2, 0);
MERGE INTO mg_inherit_target AS target USING mg_inherit_source AS source ON target.id = source.id
  WHEN MATCHED THEN UPDATE SET value = source.value;
SELECT id, value FROM mg_inherit_target ORDER BY id;
MERGE INTO ONLY mg_inherit_target AS target USING mg_inherit_source AS source ON target.id = source.id
  WHEN MATCHED THEN UPDATE SET value = source.value + 100;
SELECT id, value FROM mg_inherit_target ORDER BY id;
CREATE TABLE mg_inherit_insert_target (id integer PRIMARY KEY, value integer);
MERGE INTO mg_inherit_insert_target AS target USING ONLY mg_inherit_source AS source ON target.id = source.id
  WHEN NOT MATCHED THEN INSERT (id, value) VALUES (source.id, source.value);
SELECT id, value FROM mg_inherit_insert_target ORDER BY id;
DROP TABLE mg_inherit_insert_target;
DROP TABLE mg_inherit_target_child;
