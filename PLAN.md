...
3. **SQL and catalogs.** Close remaining language, type, privilege, collation, and introspection differences through strict PostgreSQL differential tests. Index definitions retain typed keys, `INCLUDE` metadata, partial predicates, and `NULLS NOT DISTINCT` semantics across transactions, WAL, checkpoints, catalogs, and uniqueness checks. Global, tablespace, and concurrent variants remain explicit gaps. Aggregate window `FILTER` is supported; PostgreSQL rejects `DISTINCT` and argument ordering in a window call.
...
3. **DDL and catalogs.** Typed column, domain, enum, sequence, schema, view, routine, and index definitions are transaction-owned state across DDL, WAL, recovery, catalogs, and query description. Index renames, predicates, `INCLUDE` columns, and `NULLS NOT DISTINCT` follow one durable path through rollback, checkpoints, and `pg_indexes`; remaining catalog work is broader PostgreSQL coverage. Functions and procedures retain overload identity, owner, and ACLs across replacement, identity changes, recovery, and introspection. Synthesized results are statement-arena bounded and fail on exhaustion.
