...
| WAL compatibility | Pos3ql WAL is durable, checksummed, LSN-ordered, and cold-recoverable from object storage; an object-store upload failure fences subsequent statements and startup reconciliation must complete before admission. Replication startup is explicitly authorized; logical sessions retain PostgreSQL's simple-query SQL capability while both replication modes expose `IDENTIFY_SYSTEM` with the protocol's exact types; and the wire has a distinct CopyBoth response. Publications now have transactional DDL, catalog rows, journal replay, and checkpoint/cold-start recovery. | Implement the logical publisher stream (`START_REPLICATION`, `pgoutput`) over those durable publications, then subscriber migration. PostgreSQL physical XLOG remains outside this design. |
