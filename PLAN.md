...
- SQL function programs type every formal through CTE, derived, and set-query descriptions. Every preceding query or DML step runs; the final query or DML `RETURNING` statement supplies the result, and `void` returns typed NULL after its final action. A mutable scalar call can occur in a projection, predicate, or join without invalidating the scan source image or repeating earlier sequence effects. Scalar calls cannot resolve set-returning definitions, and table calls cannot resolve scalar definitions. `RETURNS TABLE` owns ordered result names and types through replacement, WAL, checkpoints, catalogs, and scans.
