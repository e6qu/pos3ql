    ...
    for (i, v) in values.iter().enumerate() {
        ...
        match v {
            ...
            Datum::Regtype {
                ...
                rest[..4].copy_from_slice(&referenced_oid.to_le_bytes());
...
pub(crate) fn decode<'a>(
    ...
    for i in 0..n {
        ...
        match schema[i] {
            ...
            ColType::Int4 | ColType::Int2 | ColType::Oid | ColType::Xid => {
                ...
                out[i] = match schema[i] {
                    ...
                    ColType::Oid | ColType::Xid => {
                        Datum::Oid(u32::from_le_bytes(b.try_into().unwrap()))
                    }
                    _ => Datum::Int4(x),
                };
                at += 4;
            ...
            ColType::Regtype => {
                ...
                let raw = bytes.get(at + 8..at + 8 + length).ok_or_else(corrupt)?;
            ...
            ColType::Jsonpath => {
                ...
                let raw = bytes.get(at..at + len).ok_or_else(corrupt)?;
                at += len;
                out[i] = Datum::JsonPath(core::str::from_utf8(raw).map_err(|_| corrupt())?);
            }
            ...
            | ColType::PgMcvList
            | ColType::PgStatisticArray
            | ColType::AnyArray => return Err(corrupt()),
        }
    }
    Ok(())
}
