...
class DuplicateCodeTests(unittest.TestCase):
    ...
    def test_ignores_explicitly_generated_source(self):
        marker = "//! @generated by tools/generate-unicode-tables.py; do not edit.\n"
        body = "\n".join(f"let value_{number} = {number};" for number in range(4))
        result = self.run_check({"one.rs": body, "generated.rs": marker + body})
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
