...
4. **Wire and bulk data.** In progress: typed text/binary Bind and nested-record input share catalog resolution, enforce JSON and structured-wire boundaries, and preserve domain constraints. Binary COPY query output includes anonymous records. Set-returning integer, numeric, and temporal output types share one Result/Describe boundary. Close remaining accepted-type typmod, portal, and driver behavior.
