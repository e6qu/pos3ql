...
3. **DDL and catalogs.** Typed column, domain, enum, sequence, schema, view, routine, and index definitions are transaction-owned state across DDL, WAL, recovery, catalogs, and query description. Index renames, expression keys, predicates, `INCLUDE` columns, and `NULLS NOT DISTINCT` follow one durable path through rollback, checkpoints, conflict handling, and `pg_indexes`; remaining catalog work is broader PostgreSQL coverage. Functions and procedures retain overload identity, owner, and ACLs across replacement, identity changes, recovery, and introspection; a procedure body is a validated statement program whose internal results never escape `CALL`. Synthesized results are statement-arena bounded and fail on exhaustion.
