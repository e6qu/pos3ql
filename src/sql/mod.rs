...
mod tests {
    ...
    fn array_from_subquery_and_array_to_string() {
        ...
        assert_eq!(run_txn(&mut e, &mut b, &mut t,
            "SELECT array_to_string(ARRAY[1,NULL,3], ',', '*')").contains("1,*,3"), true);
        assert_eq!(run_txn(&mut e, &mut b, &mut t,
            "SELECT array_to_string(ARRAY[1,NULL,3], ',')").contains("1,3"), true);
    }
    ...
    fn trig_and_rounding_functions() {
        ...
        assert_eq!(r(&mut e, &mut b, "SELECT sinh(1)"), ["1.1752011936438014"]);
        assert_eq!(r(&mut e, &mut b, "SELECT cosh(0)"), ["1"]);
        assert_eq!(r(&mut e, &mut b, "SELECT tanh(0)"), ["0"]);
        assert_eq!(r(&mut e, &mut b, "SELECT cot(1)"), ["0.6420926159343308"]);
