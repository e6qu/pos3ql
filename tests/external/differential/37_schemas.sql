...
-- Schema tools consume the complete standard columns relation, including
-- modifiers and identity metadata rather than only names and types.
CREATE TABLE information_schema_column_detail (
  limited varchar(12), exact numeric(9,3), happened timestamp(2),
  identifier bigint GENERATED BY DEFAULT AS IDENTITY
    (START WITH 7 INCREMENT BY 3 MINVALUE 4 MAXVALUE 40 CYCLE)
);
SELECT column_name, character_maximum_length, numeric_precision,
       numeric_precision_radix, numeric_scale, datetime_precision, data_type,
       udt_schema, udt_name, is_identity, identity_generation,
       identity_start, identity_increment, identity_maximum, identity_minimum,
       identity_cycle,
       is_generated, dtd_identifier, is_updatable
  FROM information_schema.columns
 WHERE table_name = 'information_schema_column_detail'
 ORDER BY ordinal_position;
DROP TABLE information_schema_column_detail;
