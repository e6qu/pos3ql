...
3. **DDL and catalogs.** In progress: bytewise collations; typed constraint metadata; ACL-derived relation and usage grants; and sequence metadata match PostgreSQL. Column, domain, enum, sequence, schema, and view definitions are transaction-owned typed state across DDL, WAL, checkpoints, catalog rendering, and view description; staged sequence values follow the same ownership through savepoints. `information_schema.columns`, domains, constraints, and domain/type usage render from that state, including modifiers, named checks, generated columns, identities, and UDT identity; `information_schema.tables` shares transaction visibility, and synthesized index/foreign-key catalogs are arena-sized rather than silently capped. Login verification has explicit bootstrap, stored-role, and rejection states. Finish remaining client-tool introspection.
