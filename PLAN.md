...
| WAL compatibility | Pos3ql WAL is durable, checksummed, LSN-ordered, and cold-recoverable from object storage; an object-store upload failure fences subsequent statements and startup reconciliation must complete before admission. Each committed nonempty transaction ends in an explicit durable boundary, so recovery and replication consumers cannot mistake a torn operation tail for a committed transaction. Replication startup is explicitly authorized; logical sessions retain PostgreSQL's simple-query SQL capability while both replication modes expose `IDENTIFY_SYSTEM` with the protocol's exact types; the wire has a distinct CopyBoth response. Publications and bounded logical replication slots now have journal replay, checkpoint/cold-start recovery, and their catalog surfaces; `CREATE_REPLICATION_SLOT ... LOGICAL pgoutput` creates its durable resume point only after the WAL commit boundary. | Implement the logical publisher stream (`START_REPLICATION`, `pgoutput`) over durable publications, slots, and transaction boundaries, including standby status acknowledgements that advance slot positions; then subscriber migration. PostgreSQL physical XLOG remains outside this design. |
