...
There are no known unresolved bugs. Last reviewed: 2026-08-27 against PostgreSQL 18.6 across query semantics, creation-bound and string-body routines, DML overload binding and dependency lifecycles, non-atomic PL/pgSQL `CALL`/`DO` transaction boundaries and non-replayable waits, scoped routine configuration and authorization, accepted types, portals, drivers, COPY, catalogs, WAL, checkpoints, and provider-neutral cold recovery. Details belong in tests and git history, not this blocker register.
